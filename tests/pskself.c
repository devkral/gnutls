...
void doit(void)
{
	...
	run_test("NORMAL:-VERS-ALL:+VERS-TLS1.3", 0);
	run_test("NORMAL:-VERS-ALL:+VERS-TLS1.3:-GROUP-ALL:+GROUP-FFDHE2048", 0);
	run_test("NORMAL:-VERS-ALL:+VERS-TLS1.3:-GROUP-ALL:+GROUP-SECP256R1", 0);
	/* the follow should work once we support PSK without DH */
	/*run_test("NORMAL:-VERS-ALL:+VERS-TLS1.3:-GROUP-ALL", 0);*/
